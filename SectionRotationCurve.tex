...
\frac{V_R (\RGC) \left(R_\odot \cos \glon + (1 - 2 \cos^2 \glon) d  \right) }{\RGC} - V_{\phi, \odot} \sin \glon + V_{R, \odot} \cos \glon,
\end{equation}
...
The magnitude of the line-of-sight velocity dispersion corresponding to a given $\sigma_\phi$ and $\sigma_R$ is
\begin{equation}
text
\end{equation}

Since we have measurements over large ranges in Galactic longitude and heliocentric distance, we can attempt to infer not only $V_\phi(\RGC)$ and but also $R_\odot$, $V_{\phi, \odot}$, and $V_{R, \odot}$. 
