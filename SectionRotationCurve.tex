...
The biggest potential pitfall is that we have assumed departures from circular rotation are spatially uncorrelated.
The degree to which this is a problem depends on the scale on which the departures actually are correlated.
If this scale is small compared to the size of the region we are modeling, we may overestimate $\sigma_\phi$ or $\sigma_R$, but should otherwise be fine.
If the scale is smaller than the size of the region but of the same order, the region is likely to contain more departures in one direction (e.g., positive) than in the other (in this case, negative).
Finally, if the scale and is larger than size of the region, the values we derive will not actually apply to the Galaxy -- they will apply to the departure region.
