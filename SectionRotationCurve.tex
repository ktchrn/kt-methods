...
We will also assume that there are no azimuthally constant radial flows, so $V_R(\RGC) \equiv 0\text{ km/sec}$.

Since we have measurements over large ranges in Galactic longitude and heliocentric distance, we can attempt to infer not only $V_\phi$ and $V_R$ but also $R_\odot$, $V_{\phi, \odot}$, and $V_{R, \odot}$.
