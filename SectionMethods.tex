...
\label{eq:radius}
R_{i,j,k} = \sqrt{ 
    R_\odot^2 + (d_k \cos b_j)^2 - 2 R_\odot d_k \cos b_j \cos \ell_i
}
\end{align}

\begin{align}
\label{eq:velo-los-phi}
v_{\rm{los}, \phi}(v_\phi; \ell_i, b_j, d_k) = \sin \ell_i \cos b_j 
 \left( v_\phi \frac{R_\odot}{R_{i,j,k}} - v_{\phi, \odot} \right)
\end{align}

\begin{align}
\label{eq:velo-los-dispersion}
v_{\rm{los}, R} (v_R; \ell_i,  b_j, d_k) = v_R \frac{\cos b}{R_{i,j,k}} 
\left( 
\left( 1 - 2 \cos^2 \ell_i \right) d_k \cos b_j + R_\odot \cos \ell_i
\right)
\end{align}
